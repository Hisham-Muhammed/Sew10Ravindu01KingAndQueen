...
function Sewqueen(info, func) {
    ...
    var infos = {
        ...
        dontSewqueenList: info['dontSewqueenList'] === undefined ? false : info['dontSewqueenList'],
        warn: info['warn'] === undefined ? '' : info['warn'],
        function: func
    };

    if (info['on'] === undefined && info['Sewqueen'] === undefined) {
        infos.on = 'message';
        infos.fromMe = false;
    } else if (info['on'] !== undefined && types.includes(info['on'])) {
        infos.on = info['on'];

        if (info['Sewqueen'] !== undefined) {
            infos.Sewqueen = new RegExp((info['handler'] === undefined || info['handler'] === true ? config.HANDLERS : '') + info.Sewqueen, (info['flags'] !== undefined ? info['flags'] : ''));
        }
    } else {
        infos.Sewqueen = new RegExp((info['handler'] === undefined || info['handler'] === true ? config.HANDLERS : '') + info.Sewqueen, (info['flags'] !== undefined ? info['flags'] : ''));
    }
